class GameManager {
  ...
  GameManager(int frameWidth, int frameHeight) {
    this.frameWidth = frameWidth;
    this.frameHeight = frameHeight;

    // Crear el cañón
    cannon = new Cannon(100, frameHeight - 100, frameWidth, frameHeight);
  ...
  void display() {
    ...
    fill(0); // Negro
    rect(0, 0, frameWidth, 20); // Borde superior
    rect(0, frameHeight - 20, frameWidth, 20); // Borde inferior
    rect(0, 0, 20, frameHeight); // Borde izquierdo
    rect(frameWidth - 20, 0, 20, frameHeight); // Borde derecho
    ...
    void checkAndResolveCollision(Ball b1, Ball b2) {
        ...
        if (distance < minDistance && distance > 0) {
            ...
            float restitutionCoefficient = min(b1.restitution, b2.restitution);
  ...
  void displayDebugInfo() {
    ...
    rect(frameWidth - 220, 20, 200, 160);

    fill(255); // Texto en blanco
    textSize(14);
    textAlign(LEFT, TOP);
    text("Debug Mode:", frameWidth - 210, 30);
    text("Gravity: " + (gravityEnabled ? "ON" : "OFF"), frameWidth - 210, 50);
    text("Gravity Force: " + gravityForce, frameWidth - 210, 70);
    text("Ball Count: " + balls.size(), frameWidth - 210, 90);
    text("Collision Count: " + collisionCount, frameWidth - 210, 110);
    text("Auto Mode: " + (cannon.autoMode ? "ON" : "OFF"), frameWidth - 210, 130);
    text("Collector Balls: " + collector.getBallCount(), frameWidth - 210, 150);
  }
